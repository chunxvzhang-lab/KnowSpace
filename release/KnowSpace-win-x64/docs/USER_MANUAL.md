...
| v2.6.3 | 导图顶栏重排：双栏展开时两行对齐成一列 | **思维导图**：顶栏由"会自己换行的横排"改为**网格** —— 标题 ｜ 控件区 ｜ 看什么 / 带走什么；设置控件（搜索 / 主题 / 布局 / 编号）装不下时**整排**落到第二行，并与第一行第一个控件**同列**、可伸展到右端；缩放与导出固定在**第一行右上**，不再被挤到第二行末尾；一行还是两行由**导图区自己的宽度**决定（容器查询，宽过 1800px 回到一行）；控件统一 30px 高、组间 6px / 组内 4px，各窄档位补回上下留白，主题与布局下拉做成真正的控件（此前看着像被禁用）；测试 **99 套件 / 1241 用例** |
