...
| 适用软件版本 | **v2.6.5**（`package.json` 版本号，对话框与本文档一致） |
| 适用平台 | Windows 10 / 11（x64） |
| 文档类型 | 功能全覆盖操作手册（含操作案例、操作步骤、预期结果、注意事项） |
| 编写依据 | 项目源码 `src/`、`electron/`，发行说明 `docs/RELEASE_NOTES_v2.4.0.md`、`docs/RELEASE_NOTES_v2.5.0.md`、`docs/RELEASE_NOTES_v2.6.0.md`，测试基线 `docs/TEST_BASELINE.md` |
...
- [一、项目概述](#一项目概述)
  ...
  - [1.2 技术架构](#12-技术架构)
  - [1.3 版本演进（v2.2 → v2.6.5）](#13-版本演进v22--v261)
...
| v2.6.0 | 导图导入导出补齐 · 闪卡的来源、撤销与写回安全 | **思维导图**：新增 `.xmind` 导出与导入（自写 DEFLATE 解压 + ZIP 读写），导出共 **7 种**（PNG / SVG / 打印即 PDF / OPML / FreeMind / `.xmind` / Markdown 大纲），导入 **3 种**（OPML / FreeMind / `.xmind`）；**闪卡**：四种卡片来源（闪念 Space / 当前知识库 / 自定义文件夹（**最多 5 个**）/ 当前文档）并记住上次用过的来源、评分**可撤销一步**（`Ctrl/Cmd + Z`）、命令面板一步「开始复习」、斜杠命令 `/闪卡` 三条模板、评分写回**合并进文件当下内容**（不覆盖别处的改动，已删卡片不再写回）；**修复**：知识库来源改为**按路径配对**（此前会把进度**写进别的文档**）、同一张卡出现在两篇笔记时被重复计数、撤销唯一一次评分会留下空行 |
| v2.6.5 | 隐藏文件开关 · 目录不再漏文件 · 复盘与切换提速 | **新增「显示点开头的文件」开关**（工具栏，仅在打开了文件夹时出现；默认**关**，升级后第一眼与 v2.6.4 一致）：工具与系统目录（`.git`、`node_modules`、`dist`、`$RECYCLE.BIN` 等）**始终跳过**，开关不会把它们带出来；读者自己的隐藏文件（`.草稿.md`、`.archive/`）在开关打开时显示并弱化呈现，**排序上作为一整块排在可见项之后**（打开开关是「末尾多出一段」，不是把列表重排一遍），判定规则为「文件名有点，或它上面任意一层文件夹有点」；**修复目录扫描静默漏文件**：文件上限 3000 / 深度 6 层两处硬截断改为护栏（50000 / 64 层 + 20 秒墙钟兜底），**截断原因必须上报**并在树底部提示，补 junction 环路保护与符号链接分支，并区分「打不开的目录」与「空目录」（此前 `EACCES/EPERM/ENOENT` 一律返回空列表）；**修复复盘文件夹列表滚不动**（列表无滚动容器，外层 `overflow:hidden` 直接裁掉；并修复空列表时「添加文件夹」入口被一并藏掉）；**修复打开新文件的视觉跳动**（同步查渲染缓存使「切换身份」与「换上新正文」落在同一次 React 提交，缓存 12 → 40 篇 + 48 MB 字节上限，空闲预取相邻文档，切换淡入以**文档标识**而非内容校验和为键）；**性能**：围栏扫描由每行重扫（O(n²)）改为一次预扫描（5000 行 **973.6ms → 1.01ms**）、文件夹路径去重改用 `Set`（5000 路径 **7080ms → 4.4ms**，此前选满 5 个文件夹切来源会僵住约 7 秒）、目录树构建改 Map 查找（9000 篇 **265ms → 23ms**）、索引通知由每文档一次改为每批一次；测试 **110 套件 / 1382 用例** |
| v2.6.4 | 白板卡片：斜杠命令侧边栏与滚轮归属 | **白板**：卡片编辑器键入 `/` 弹出与文档编辑器**同一套**斜杠命令（此前 `/` 打开的是「引用笔记」弹层，且两个编辑器各写一份触发规则，必然分叉）；**引用笔记改用 `[[`**，与文档编辑器的语义一致；从命令列表选 `[[]]` 会**自动接上笔记列表**；**滚动卡片内的清单不再带动画布** —— 滚轮按**滚动链语义**判定归属（内层还有得滚就归内层，滚到头还给画布），取代此前「清单在滚、画布也在滚」；**修复**：`DailyReviewPanel` 在全量测试下的偶发失败（复习来源写入 `localStorage` 却未在测试之间清理）；测试 **103 套件 / 1276 用例** |
| v2.6.3 | 导图顶栏重排：双栏展开时两行对齐成一列 | **思维导图**：顶栏由"会自己换行的横排"改为**网格** —— 标题 ｜ 控件区 ｜ 看什么 / 带走什么；设置控件（搜索 / 主题 / 布局 / 编号）装不下时**整排**落到第二行，并与第一行第一个控件**同列**、可伸展到右端，缩放与导出整组搬到**第二行右端**（第二行不再空半行）；一行还是两行由顶栏**按实测宽度自行决定**（六种排法各量一遍，第一种放得下的胜出；搜索框开闭、多选、标题长短都算得进去，取代写死的 1800px 阈值）；控件统一 30px 高、组间 6px / 组内 4px，各窄档位补回上下留白，主题与布局下拉做成真正的控件（此前看着像被禁用）；节点上的**链接徽章改为透明底**（只剩箭头与细边，SVG 导出同步）；测试 **99 套件 / 1241 用例** |
| v2.6.2 | 导图细节打磨：顶栏对齐、徽章真能点、图标位置与 PNG 清晰度 | **思维导图**：顶栏两行布局下**第二行与第一行对齐**（此前各组基线不同，右侧那排看起来是错位的）；**链接徽章真正可点** —— 根因是父层的 `pointer-events: none` 被继承下来、徽章收不到点击，现改为显式 `auto`，并支持**裸域名**（`example.com`）；节点类型图标改画在**主题盒子之上**，不再被连线压住；**PNG 导出改为 3× 超采样**，放大后文字与图标不再发虚；测试增至 **99 套件 / 1241 用例** |
| v2.6.1 | 导图交互收口：顶栏、按键归属、链接徽章与节点类型 | **思维导图**：顶栏改为**装不下就换行**（打开文件目录与大纲后右侧控件不再消失），**缩放与导出**归右侧同一组，滚轮落在顶栏上不再缩放画布；**快捷键归属插入符所在的输入框**（备注框里的 Delete 不再删掉整个主题、标签或链接框里的 Ctrl+V 不再贴出一个分支），节点菜单新增**复制/剪切整个主题、粘贴为子主题**三行，删除行改为「删除整个主题及其子主题」；节点图标重做为**节点类型**（一行 8 枚、一名一义、点击设置再点取消、按类型名搜索可找出同类主题，旧文件写过的图标 id 仍会画出）；节点**链接徽章可直接点击打开**（自由主题上的同样）；点击顶栏**收起右键菜单**且不像点画布那样连选择一起丢掉；「换到另一侧」弹层两行字对比度提升；测试增至 **99 套件 / 1238 用例** |
...
- 测试：**94 个测试套件 / 1208 项用例，100% 通过**（基线快照见 `docs/TEST_BASELINE.md`，可按 `node scripts/capture-test-baseline.cjs` 重新生成）。
- TypeScript：构建期 0 错误。
- 其中闪卡覆盖为 **v2.5.0 引入**（调度与语法解析 `fsrs-service.test.ts` 70 项、复盘面板 `daily-review-panel.test.tsx` 38 项）与 **v2.6.0 扩展**（`fsrs-verification.test.ts`、`review-sources.test.tsx`、`review-source-reader.test.ts`、`review-focus.test.tsx`：来源切换、按路径配对、撤销与写回合并）。
...
1. 下载 `KnowSpace-Setup-2.6.5.exe`。
2. 双击运行，按向导选择安装目录（默认当前用户安装，允许提权）。
3. 完成后勾选「运行 KnowSpace」，或从桌面快捷方式/开始菜单启动。

**预期结果**：桌面与开始菜单出现 KnowSpace 图标；系统级 `.md`、`.canvas` 文件关联就绪；控制面板中出现卸载项 `KnowSpace 2.6.5`。

#### 方式二：MSI 标准安装包（适合企业分发）

```powershell
msiexec /i KnowSpace-2.6.5.msi /qn
```
...
- **升级**：直接运行新版本安装包覆盖安装即可，**无需迁移动作**（v2.6.5 未改变任何既有文件格式，也不新增强制目录）。新版本创建的库可被旧版本打开，多出的注释块与普通注释无异。
...
| 15 | **FSRS-5 间隔重复闪卡** | 命令面板「开始复习」/ Space 面板 →「复盘」 | 让读过的内容真正记住 | [36](manual-images/36-fsrs-card-syntax.png) [37](manual-images/37-fsrs-review-question.png) [38](manual-images/38-fsrs-review-answer.png) | [4.15](#415-fsrs-5-间隔重复闪卡v250-新增) |
...
以 `.` 开头的文件与文件夹，v2.6.4 及以前**一律不显示**且没有开关。v2.6.5 起工具栏多了一个开关，
但只对**你自己的文件**生效 —— 分两类：

| 类别 | 例子 | 行为 |
| :--- | :--- | :--- |
| 工具与系统目录 | `.git`、`.svn`、`node_modules`、`dist`、`release`、`$RECYCLE.BIN`、`System Volume Information` | **始终跳过**，开关打开也不显示 |
| 你自己的隐藏文件 | `.草稿.md`、`.templates/`、`.archive/旧稿.md` | 开关打开时显示，并以弱化样式（降透明度 + 斜体）呈现 |

**入口**：工具栏「显示点开头的文件」按钮（`Eye` 图标）。该按钮**仅在打开了文件夹时出现** ——
单开一个文件时没有目录树，也就没有它要影响的东西。设置会记住，下次打开文件夹仍然生效。

**判定规则**：文件名以点开头，**或它上面任意一层文件夹以点开头**。
`.archive/旧稿.md` 属于隐藏，即使它自己的名字没有点 —— 是读者把文件夹藏起来的。

**排序规则**：隐藏项在同一层里作为**一整块**排在可见项**之后**，而不是按名字与可见项混排。
文件夹在可见、隐藏两个区块内部依然排在文件前面。这样做是因为打开开关应当是「末尾多出一段」，
而不是把正在看的列表重新排一遍 —— 文档挪位置只因为**另一个**文档被显示出来，是让列表显得
不可靠的那类变化。该顺序同时决定复习队列里文档的先后。

**操作步骤**

1. 打开一个文件夹（`Ctrl+Shift+O`）。
2. 观察工具栏出现「显示点开头的文件」按钮，此时它未激活。
3. 点击该按钮，目录树底部多出一段弱化的隐藏条目。
4. 点击其中一个隐藏文档，确认可以正常打开、编辑。
5. 再次点击按钮，隐藏条目消失，且**可见文档的顺序与第 2 步完全一致**。

**注意事项**

- 默认**关闭**，因此升级到 v2.6.5 后第一眼看到的目录树与 v2.6.4 完全一样。
- 隐藏与否只影响**显示**，不影响读写：隐藏文件可以正常打开、编辑、参与双链、进入复习队列。
- 扫描上限已放宽到 50000 个文件 / 64 层；若某次列目录确实被截断或存在打不开的目录，
  目录树底部会出现一条说明，而不是像以前那样静默少列。
...
3. 点击顶栏「导出导图 ▾」，选择：**PNG（3× 超采样，透明底，放大后文字与图标不虚）**、**OPML 2.0**、**FreeMind 1.0.1 (.mm)**、**`.xmind`**、**SVG**、**打印即 PDF** 或 **Markdown 大纲**。
...
- 根节点受防误删保护。
- **节点类型**：一个主题至多一枚（右键菜单「节点类型」，8 枚一名一义 —— 待办 / 进行中 / 已完成 / 疑问 / 想法 / 风险 / 重点 / 参考），点击设置、再点同一枚取消；在导图搜索框里输入类型名（如「待办」）即可找出所有同类主题。旧文件里写过的旧图标 id 仍会画出、也仍能清除。
- **链接徽章可点**：节点上的链接徽章直接点击即打开（外链走系统浏览器，`[[笔记]]` 与 `#标题` 走应用内跳转），且不会顺手选中或拖动主题。
- **快捷键归属插入符所在的输入框**：插入符停在备注 / 标签 / 链接框里时，`Delete`、`Ctrl+V` 等键属于那个框（不再删掉整个主题或贴出一个分支）；`Escape` 是唯一例外。
- **顶栏装不下就换行**：打开文件目录与大纲后右侧控件不会被推出可视区 —— 两行布局下，设置控件（搜索 / 主题 / 布局 / 编号）整排落到第二行并与第一行第一个控件对齐，缩放与导出留在第一行右上；导图区宽过 1800px 时回到一行。滚轮落在顶栏上不缩放画布。
...
- 双击文本卡片进入 Markdown 行内编辑；`Ctrl/Cmd + Enter` 保存，`Esc` 取消，点击卡片外也会保存。
- 编辑时键入 `/` 弹出**斜杠命令列表** —— 与文档编辑器**同一套命令、同一条触发规则**（命令表定义在 `src/services/slashCommands.ts`，两个编辑器共用）：`↑` `↓` 选择，`Enter` 或 `Tab` 插入，`Esc` 关闭；支持英文、拼音首字母与中文搜索（`/todo`、`/db`、`/闪卡` 都可以）。
- 键入 `[[` 弹出**工作区笔记列表**，`Enter` 插入 `[[笔记标题]]`；从命令列表里选「双向链接 `[[]]`」会**自动接上**这个列表，和文档编辑器一致。
- `/` 只在一行开头或空格之后触发 —— 路径（`notes/a.md`）、网址（`https://…`）与 `//` 注释里的斜杠**不会**弹出任何面板。
- **卡片内滚动不会带动画布**：滚轮落在卡片正文或引用面板上时，归该区域自己滚动；滚到顶或底之后，滚轮才交还给画布继续平移。按住 `Ctrl` 滚动始终是画布缩放。
...
**最快的方式：斜杠命令。** 在编辑器里输入 `/闪卡`（或 `/复习`），菜单里有三条 —— 「闪卡：问答」「闪卡：一行一问一答」「闪卡：挖空」。每条模板**插进去就已经是一张合法的卡片**，光标停在示例文字上，直接改就是你的卡片。语法不必背：菜单里的描述就写着这张卡之后会怎么被问（包括最容易漏掉的一条 —— `==高亮==` 也算挖空）。

**操作步骤（写下第一张闪卡）**

1. 打开 `Space/` 目录下的任意闪念文件，或任意一篇笔记（复习时可以从四个来源里选，见 4.15.2）。
2. 输入以下内容：
...
- 左侧活动栏「闪念 Space 时间线」→ 顶部标签「**复盘**」；
- 命令面板（`Ctrl/Cmd + K`）→「**开始复习：闪卡**」—— 一步直达，会自己把侧栏打开到复盘。

**卡片来源**：面板顶部来源行有四档，切换即换一轮。

| 来源 | 读什么 | 说明 |
| :--- | :--- | :--- |
| 闪念 Space | `Space/` 目录下的闪念 | 默认；打开面板即用 |
| 当前知识库 | 已打开工作区的全部章节 | 第一次切过去时才读，此后记着 |
| 自定义文件夹 | 最多 **5 个**你挑的文件夹 | **只是复习，不会把它们打开成工作区**；选择被记住（记住的是路径，下次重新列一遍，所以新写的卡进得来）；每行「移除」，末行「添加文件夹」再加一个；两个文件夹里有同一个文件时那个文件只读一次 |
| 当前文档 | 正在编辑的那一篇 | **有未保存改动时不可选**并说明原因：评分要写进文件，而没有自动保存，之后那次保存会把进度覆盖掉 |

面板会**记住上次用的来源**，下次打开停在那里；记住的那个这次用不了时（知识库没开、文件夹已取消、文档未保存）自动退回 Space。
...
**评分可以撤销一步**：按错了（`重来` 与 `良好` 只隔一个键位）就点标题右侧的撤销按钮，或按 `Ctrl/Cmd + Z`（在输入框里按仍是编辑器自己的撤销）。撤销把**评分前的调度写回去**；若那张卡此前从没复习过，则**把那一行整个收回** —— 一篇笔记若只评过一次就被撤销，它会与从未评过时**逐字节相同**。同时那张卡**回到本轮**（调度被收回了，就等于没有复习过）。按钮只在有分可撤时出现。

**注意事项**

- 只有 `due` 日期 ≤ 今天的卡片才会进入队列；未到期的卡片会明确提示「没有到期的卡片。今日仍有 N 张已在计划中」。
- **评分只改自己那一行，不改整篇**：写回前会重新读一次文件，把这次评分合并进**文件当下的内容**。因此复习期间在别处（编辑器、同步盘、闪念胶囊）对同一文件的改动**不会被覆盖**；反过来，复习期间被删掉的卡片，它的进度行也会被丢弃，而不是写给一个已经不存在的卡片。读不到文件时（例如盘符暂时不可用）会明确报错并**留在原地**，不会拿手里的旧副本硬写。
...
> 下表为 **v2.6.5 实际生效** 的快捷键（依据 `src/hooks/useGlobalShortcuts.ts`、`src/components/CanvasView.tsx`、`src/components/MindmapView.tsx`、`src/components/FlashCapsule.tsx`、`electron/main.cjs` 核对）。标注「编辑区内」的快捷键会穿透 CodeMirror 焦点直接生效。
...
以下条目出现在早期 README 或旧手册中，但 **v2.6.5 未实现**，请勿依赖：
...
A：**不会**。写回前会重新读一次文件，把这次评分**合并进文件当下的内容** —— 它改动的是自己那一行调度元数据，别的内容（包括你刚在别处写下的段落）原样保留。两个方向都成立：复习期间被删掉的卡片，它的进度行会**被丢弃**而不是写给一个不存在的卡片；而万一文件读不到（盘符暂时不可用等），会**明确报错并留在原地**，不会拿手里的旧副本硬写。
...
| 2026-09-19 | v2.6.0 | 随 v2.6.0 发布同步。本手册成为**唯一正式版本**：文件更名为 `docs/USER_MANUAL.md`（此前按版本号命名的同名文件不再保留），README 与桌面打包均指向这一份，结束"手册有两份、打包带走旧的那份"的状态。内容上：4.15 补齐四种卡片来源 / 一步撤销（`Ctrl/Cmd + Z`）/ 命令面板入口 / 斜杠命令 `/闪卡` / 最多 5 个自定义文件夹 / 写回合并进文件当下内容；FAQ 的 Q26 改写为"**不会**覆盖别处改动"；质量基线、版本引用与安装包文件名同步至 v2.6.0 |
| 2026-09-20 | v2.6.3 | 随 v2.6.3 发布同步：本次未改变任何既有文件格式。新增 1.3 变更表的 v2.6.3 行；4.12 补上顶栏的一行 / 两行口径（两行时设置控件与第一行第一个控件同列，缩放与导出留在右上；导图区宽过 1800px 回到一行）；其余版本引用由 scripts/bump-doc-version.py 提升 |
| 2026-09-20 | v2.6.2 | 随 v2.6.2 发布同步：本次未改变任何既有文件格式。新增 1.3 变更表的 v2.6.2 行；4.12 首次把 v2.6.1 起实际可用的三类交互写进正文（**节点类型**、**链接徽章可点**、**快捷键归属插入符所在的输入框**）并补上「顶栏装不下就换行」；4.12 的 PNG 导出口径由 2× Retina 更正为 **3× 超采样**（与实现一致）；其余版本引用由 scripts/bump-doc-version.py 提升 |
| 2026-09-20 | v2.6.1 | 随 v2.6.1 发布同步：本次未改变任何既有文件格式；新增 1.3 变更表与附录的 v2.6.1 行；版本引用由 scripts/bump-doc-version.py 提升，其中「版本演进」一条改为匹配任意上一次版本（此前写死，改一次就得手工维护脚本） |
| 2026-09-21 | v2.6.4 | 随 v2.6.4 发布同步：本次未改变任何既有文件格式。新增 1.3 变更表的 v2.6.4 行；4.13.1 新增「卡片内编辑与引用」小节（`/` 打开命令、`[[` 引用笔记、以及卡片内滚动不带动画布）；其余版本引用由 scripts/bump-doc-version.py 提升 |
| 2026-09-26 | v2.6.5 | 随 v2.6.5 发布同步：本次未改变任何既有文件格式。新增 1.3 变更表的 v2.6.5 行；4.2 新增「隐藏文件」小节（开关位置、两类过滤规则、排序规则、判定规则）；版本引用由 scripts/bump-doc-version.py 提升 |
