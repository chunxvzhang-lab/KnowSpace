...
> ⚠️ **本文档已归档 · 请阅读 `docs/RELEASE_PLAN_v2.5.0.md`**
>
> **归档原因**：本文制定于 2026-09-16，将 `v2.4.0` 规划为「架构解耦 + FSRS 闪卡」。但在计划窗口启动前，`v2.4.0` **已实际发布**，内容为白板交互优化与导出链路加固（即本文 §2.3 列出的 16 项累积修复）。
>
> 因此**本文的全部目标（R1 解耦 / R2 拆分 / F1 FSRS / R4 治理）已整体顺延至 `v2.5.0`**，并由 `RELEASE_PLAN_v2.5.0.md` 承接。新版计划基于实测校准了基线（原记载的 `CanvasView` 8451 行实为 8636 行、测试 379/299 实为 384 项，并新识别出 `canvasService.ts` 4535 行需一并拆分）。
>
> 本文**保留作为历史记录**，不再作为执行依据。

> **文档性质**：下一版（v2.4.0）可执行发布计划
> **制定时间**：2026-09-16
> **上一版基线**：`v2.3.0`（已发布 MSI / NSIS / 便携版）
> **目标版本**：`v2.4.0` → **已顺延为 `v2.5.0`**
