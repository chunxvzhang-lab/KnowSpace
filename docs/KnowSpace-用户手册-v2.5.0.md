...
| v2.5.0 | FSRS-5 闪卡与架构减负 | **FSRS-5 间隔重复闪卡**（三种零侵入语法 + 斜杠命令 `/闪卡`、DSR 调度、四档评分与间隔预览、**可撤销一步**、进度存于单条 HTML 注释）；**四种卡片来源**（Space / 知识库 / 自定义文件夹（**可有多个**）/ 当前文档）并记住上次用的那个，命令面板可一步「开始复习」；评分写回**合并进文件当下内容**（不覆盖别处的改动）；`App.tsx` 减少 40%、`canvasService` 拆为 9 模块 + 门面、`CanvasView` 减少 30%；测试增至 **94 套件 / 1208 用例** |
