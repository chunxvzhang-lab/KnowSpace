...
| v2.5.0 | FSRS-5 闪卡与架构减负 | **FSRS-5 间隔重复闪卡**（三种零侵入语法 + 斜杠命令 `/闪卡`、DSR 调度、四档评分与间隔预览、**可撤销一步**、进度存于单条 HTML 注释）；**四种卡片来源**（Space / 知识库 / 自定义文件夹 / 当前文档）并记住上次用的那个，命令面板可一步「开始复习」；评分写回**合并进文件当下内容**（不覆盖别处的改动）；`App.tsx` 减少 40%、`canvasService` 拆为 9 模块 + 门面、`CanvasView` 减少 30%；测试增至 **92 套件 / 1184 用例** |
...
- 测试：**92 个测试套件 / 1184 项用例，100% 通过**（基线快照见 `docs/TEST_BASELINE.md`，可按 `node scripts/capture-test-baseline.cjs` 重新生成）。
- TypeScript：构建期 0 错误。
...
| 自定义文件夹 | 你挑的一个文件夹 | **只是复习，不会把它打开成工作区**；选择被记住（记住的是路径，下次重新列一遍，所以新写的卡进得来）；行内「更换 / 取消」可随时改 |
