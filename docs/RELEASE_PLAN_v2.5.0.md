...
> 📌 **这是本轮最有指导价值的数据**：**逻辑抽取的产出约为 JSX 抽取的 1.5 倍、状态迁移的 4 倍** —— 而 `useColumnResize` 还是六个候选 hook 中**最小的一个**。
>
> 原因也清楚：hook 可以直接读 store，因此**同时消掉状态订阅与 props 透传**；而组件只能消掉 JSX 标记，props 还得留在 `App.tsx`；迁移状态则几乎是纯替换。
>
> **因此 B3b 应作为主攻方向，优先级高于继续切 JSX。**
...
| **R1-B3b-2** | 其余 5 个 hook（vault 打开 / 检索 / 反链 / 快捷键 / 书签） | 10/01 – 10/20 | R1-B3b-1 ✅ | 待启动 |
