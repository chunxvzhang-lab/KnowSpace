...
| 项目 | 状态 |
| :--- | :--- |
| `src/store/`（Zustand） | ✅ 三个 Store 已落地 —— `useUiStore`（338 行）/ `useTabStore`（191 行）/ `useVaultStore`（110 行） |
...
| 项目 | 内容 |
| :--- | :--- |
| **需求背景** | 状态散落在 `App.tsx` 中，靠 props 层层传递（Prop Drilling）。任何新功能都要改动顶层组件，回归风险高 |
| **影响范围** | 新增 `src/store/`（`useUiStore` / `useTabStore` / `useVaultStore`）；全量组件读状态路径变更 |
| **验收标准** | ① 各领域 Store 落地，领域边界清晰<br/>② **全部测试通过，零回归**<br/>③ `App.tsx` 行数 —— ⚠️ **目标已重校准，见下方**<br/>④ 无新增 `any` 逃逸 |
| **执行顺序** | ✅ B0（测试安全网 + UI 域）09/17 → ✅ B1（Tab 域 + 消除回写环）09/17 → B2（Vault 域）→ B3（JSX 分块 + 回调抽取） |
| **风险** | R-4：与 R2 同时改 `CanvasView` 相关文件 → **文件归属必须明确**（见 §5.1） |

**⚠️ 目标重校准（2026-09-17，B0 / B1 完成后实测）**

原定验收标准 `App.tsx < 500 行`。两批迁移后 **3589 → 3549 行**，据此重新评估：

| 观察 | 数据 |
| :--- | :--- |
| 已迁移状态 | 32 个 `useState`（UI 域 16 + Tab 域 4 + Vault 域 12） |
| 各批实测行数 | B0：3589 → 3554｜B1：3554 → 3549｜B2：3549 → **3554** |
| **三批净变化** | **-35 行**（3589 → 3554） |
| 选择器行数 vs 原声明行数 | **约 1:1** —— 迁移状态本身几乎不减少行数 |
| 真正被删除的代码 | 持久化 try/catch、**49 行 `isDirty` 回写 effect**、8 处 `isDirty` 字面量 |
| `App.tsx` 剩余构成 | JSX 约 **518 行** + 约 **3000 行回调与 effect** |

> 📌 **B2 是决定性的证据**：迁移 vault 域（12 个状态、约 215 处引用、含两个索引与持久化）后，`App.tsx` 行数**不降反升 5 行** —— 32 行选择器替换掉约等量的声明，说明性注释再添几行。三个不同领域各自测量一次，结论已可确定：**迁移状态在结构上就是行数中性的**。

**三类手法的实测产出对比（09/17 全部实测，非估算）**

| 手法 | 批次 | 工作量 | 行数变化 | 单位产出 |
| :--- | :--- | :--- | ---: | :--- |
| 迁移状态 → store | B0 + B1 + B2 | 32 个状态、约 350 处引用 | **-35 行** | ≈ -1 行/状态 |
| 抽取 JSX → 组件 | B3a | 浮层组 66 行 JSX | **-48 行** | ≈ -0.7 行/行 JSX |
| **抽取逻辑 → hook** | B3b | `useColumnResize`（4 个处理函数 + 1 个 effect，83 行） | **-74 行** | **最高** |

> 📌 **这是本轮最有指导价值的数据**：**逻辑抽取的产出约为 JSX 抽取的 1.5 倍、状态迁移的 4 倍** —— 而 `useColumnResize` 还是候选 hook 中**最小的一个**。
>
> 原因也清楚：hook 可以直接读 store，因此**同时消掉状态订阅与 props 透传**；而组件只能消掉 JSX 标记，props 还得留在 `App.tsx`；迁移状态则几乎是纯替换。
>
> **因此 B3b 应作为主攻方向，优先级高于继续切 JSX。** 后续三批 hook 完全印证了这个判断。

**R1 进度实测（2026-09-17，全部为实测值）**

| 批次 | 手法 | App.tsx 变化 | 增量 |
| :--- | :--- | ---: | ---: |
| 起点 | — | 3589 | — |
| B0/B1/B2 | 状态迁移（32 个状态） | 3589 → 3554 | -35 |
| B3a | JSX 抽取（`AppOverlays`） | 3554 → 3506 | -48 |
| B3b-1 | `useColumnResize` | 3506 → 3432 | -74 |
| B3b-2 | `useDocumentCreation` | 3432 → 3203 | **-229** |
| B3b-3 | `useVaultOpening` | 3203 → 2914 | **-289** |
| B3b-4 | `useSearch` | 2914 → **2586** | **-328** |
| **合计** | — | **3589 → 2586** | **-1003** |

> 📌 **状态迁移贡献 3.5%，抽取贡献 96.5%。** `App.tsx` 已减少 **28%**，其中五个 hook 与一个组件承担了绝大部分。按此速率，剩余 3 个 hook 可将 `App.tsx` 推进到 `< 1500 行` 区间 —— 目标可达且路径已验证。

**搬移脚本（可复用）**

| 脚本 | 搬移量 | 干跑发现的问题 |
| :--- | :--- | :--- |
| `extract-document-creation.cjs` | 234 行 | 依赖清单漏检 2 个符号（由 tsc 兜住） |
| `extract-vault-opening.cjs` | 295 行 | **范围误包含上一批次的 hook 调用**（会删掉它） |
| `extract-search.cjs` | 335 行 | **尾部误包含下一函数的注释**；支持双区块（非连续）搬移 |

> 📌 **三个批次各自被干跑拦下一次真实事故**，这也是脚本必须先 `--dry-run` 的实证理由。

> **结论：`< 500 行` 无法通过「迁移状态」达成。** 剩余的不是状态，而是**尚无归属的逻辑** —— 文档会话编排、检索、书签、反链、导出、快捷键等。它们必须被抽成自定义 hook 才能离开 `App.tsx`，而这超出了「三大 Store」的工作量估算。

建议把验收标准拆成两层：

| 层级 | 标准 | 可达性 |
| :--- | :--- | :--- |
| **架构（真正的目标）** | 全部领域状态离开 `App.tsx`；`App.tsx` 只负责组合与编排 | ✅ 可达 |
| **量化** | `App.tsx` **< 1500 行**（状态与 JSX 分离后） | ✅ 按 B3b 实测产出推算，六个 hook 足以达成 |
| ~~量化~~ | ~~`< 500 行`~~ | ❌ 需把约 3000 行逻辑全部抽成 hook |

> 这个结论本身有价值：它说明原计划的量化目标建立在「状态即行数」的假设上，而实际障碍在**逻辑**而非状态。B0/B1 已把「状态」这条线走完一半，剩下的行数只能靠逻辑抽取拿下。

**B3 的实测补充（09/17）**：JSX 分块已验证，**但收益集中在「状态已入 store」的区块上**。

| 区块 | 行数 | 抽取可行性 | 实测 |
| :--- | ---: | :--- | :--- |
| 浮层组（7 个对话框 + toast） | 66 | ✅ **10 个值已在 store 中** → 仅 14 个 props | ✅ 已完成，**-48 行**（`AppOverlays`，146 行） |
| 目录栏 + 侧栏面板组 | ~157 | ⚠️ 深度依赖 `isDualSplitMode` / `isCanvasFullscreen` / `manifest` / `sidebarTab` 的条件嵌套，需 20+ props | ❌ 收益低，暂缓 |
| `reader-frame`（读者区） | ~203 | ⚠️ 同上，且包含文档工作区 / 双栏 / 白板 / 图谱切换 | ❌ 收益低，暂缓 |
| `ActivityBar` / `Toolbar` / `StatusBar` | 23 / 37 / 17 | ⚠️ 各组 10-35 个 props，其中部分可读 store | ⏸ 净收益约 -10 行，不划算 |

> 📌 **B3 的结论**：JSX 只有 527 行，且剩下的大块**都绑在会话与回调上** —— 继续切 JSX 需要透传大量 props，边际收益递减。**真正值得做的是 B3b：把约 3000 行回调与 effect 抽成自定义 hook**，hook 内部直接读 store，才能同时消掉 props 与行数。

**B3b 候选抽取（按优先级排序；行数为粗估，尚未实测）**

| hook | 覆盖内容 | 预估规模 | 前置 |
| :--- | :--- | :--- | :--- |
| `useVaultOpening` | 打开目录 / 打开文件 / 桌面路径 / 新建 md·导图·白板 | 大 | 全部 store ✅ |
| `useSearch` | 检索编排、命中高亮、滚动定位 | 中 | `useVaultStore` ✅ |
| `useBacklinkIndex` | 反链索引构建与增量更新 | 中 | `useVaultStore` ✅ |
| `useGlobalShortcuts` | 快捷键与 Electron 菜单事件绑定 | 中 | 各 store ✅ |
| `useDirectoryResize` | 目录栏 / 侧栏拖拽与双击自适应 | 小 | `useUiStore` ✅ |
| `useBookmarks` | 书签增删改与定位跳转 | 小 | `useVaultStore` ✅ |

> 每个 hook 的抽取方式与 B0-B3a 一致：**先加测试、再搬移、逐批跑全量 + 基线对比**。前提条件（状态入 store）已全部就绪，这正是 B0-B2 的回报。
...
- 🔴 **09/30**：拆分设计评审通过
- 🔴 **10/20**：`App.tsx` 架构目标达成（领域状态全部离开）｜量化目标 **< 1500 行**（原 `< 500 行` 已重校准，理由见 §3.1）
...
| **R1-B0** | 测试安全网 + `useUiStore`（UI 域迁移） | 09/17 | R3 ✅ | ✅ **已完成** |
| **R1-B1** | `useTabStore` + `isDirty` 派生（消除回写环） | 09/17 | R1-B0 ✅ | ✅ **已完成** |
| **R1-B2** | `useVaultStore`（manifest / 书签 / 检索 / 反链） | 09/17 | R1-B1 ✅ | ✅ **已完成** |
| **R1-B3a** | JSX 分块（浮层组 → `AppOverlays`） | 09/17 | R1-B2 ✅ | ✅ **已完成** |
| **R1-B3b-1** | `useColumnResize`（列宽拖拽与自适应） | 09/17 | R1-B3a ✅ | ✅ **已完成** |
| **R1-B3b-2** | `useDocumentCreation`（新建 md / 导图 / 白板） | 09/17 | R1-B3b-1 ✅ | ✅ **已完成** |
| **R1-B3b-3** | `useVaultOpening`（打开文件 / 路径 / 目录） | 09/17 | R1-B3b-2 ✅ | ✅ **已完成** |
| **R1-B3b-4** | `useSearch`（检索编排 / 命中高亮 / 滚动定位） | 09/17 | R1-B3b-3 ✅ | ✅ **已完成** |
| **R1-B3b-5** | 剩余 hook（反链 / 快捷键 / 书签） | 10/01 – 10/20 | R1-B3b-4 ✅ | 待启动 |
